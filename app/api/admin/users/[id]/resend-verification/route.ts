...
export async function POST(_req: Request, context: RouteContext) {
  try {
    ...
    await adminResendVerification(id, auth.user.id);
  } catch (error) {
    ...
    console.error("ADMIN_USER_RESEND_VERIFICATION_ERROR", error);
    return NextResponse.json(
      { success: false, message: "Doğrulama e-postası isteği işlenemedi." },
      { status: 500 }
    );
  }
}
